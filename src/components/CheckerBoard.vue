<template>
  <div class="checkerboard">
    <div v-for="(line, yIndex) in squares" :key="yIndex" class="line">
      <div
      ...
      >
        <div v-if="square.selected && !square.mine" class="selected">
          {{ square.number }}
        </div>
        <div
        ...
        <div v-else-if="square.flag" class="unselected">
...
export default {
  ...
  methods: {
    ...
    setMine() {
      ...
      if (
        this.squares[y][x].mine === false &&
        !(x === this.exclude[0] && y === this.exclude[1])
      ) {
